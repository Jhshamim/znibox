...
export const searchAnime = async (keyword: string): Promise<AnimeMedia[]> => {
  const res = await fetchWithTimeout(`${NEW_API_URL}/search?q=${encodeURIComponent(keyword)}`);
  const data = await res.json();
  const movies = data.movies || [];
  return movies.map(mapSectionMovies);
};

export const fetchAnimeInfo = async (id: string): Promise<AnimeMedia> => {
  const res = await fetchWithTimeout(`${NEW_API_URL}/detail/${id}`);
  ...
  return {
    ...
    title: {
      ...
      native: metadata.title,
    },
    coverImage: {
      extraLarge: metadata.poster,
      large: metadata.poster,
      color: null,
    },
    bannerImage: metadata.poster,
    description: metadata.description,
    episodes: metadata.seasons?.[0]?.resolutions?.length || null,
    status: 'FINISHED',
    averageScore: null,
    genres: metadata.genre ? metadata.genre.split(',') : [],
    format: 'MOVIE',
    nextAiringEpisode: null,
    trailer: json.streams?.mp4?.[0] ? { id: json.streams.mp4[0], site: 'mp4' } : null,
    staff: {
      edges: (metadata.top_cast || []).map((cast: any) => ({
        role: cast.character,
        node: {
          id: cast.staffId,
          name: { full: cast.name },
          image: { large: cast.avatarUrl },
          primaryOccupations: []
        }
      }))
    }
...
export const fetchEpisodes = async (id: string): Promise<NewEpisodesResponse> => {
  const res = await fetchWithTimeout(`${NEW_API_URL}/episodes/${id}`);
  const json = await res.json();
  
  if (!json || !json.subject_id) {
    throw new Error('Failed to fetch episodes');
  }
  
  return json as NewEpisodesResponse;
};

export const fetchLanguages = async (id: string): Promise<LanguagesResponse> => {
  const res = await fetchWithTimeout(`${NEW_API_URL}/api/languages?id=${id}`);
  const json = await res.json();
  return json as LanguagesResponse;
};
...
const NEW_API_URL = 'https://movie-box-api-v1.jhshamim81.workers.dev';

export const fetchHeroBanner = async (): Promise<AnimeMedia[]> => {
  const res = await fetchWithTimeout(`${NEW_API_URL}/home/banner`);
  const data = await res.json();
  return (data.featured || []).map((item: any) => ({
    id: item.slug,
    title: {
      romaji: item.name,
      english: item.name,
    ...
    coverImage: {
      extraLarge: item.poster_url,
      large: item.poster_url,
      color: null,
    },
...
const mapSectionMovies = (item: any): AnimeMedia => ({
  id: item.slug,
  title: {
    romaji: item.name,
    english: item.name,
  ...
  episodes: null,
  status: 'FINISHED',
  averageScore: item.rating ? parseFloat(item.rating) * 10 : null,
  genres: [],
  nextAiringEpisode: null,
  format: 'MOVIE',
});

export const fetchMoviesNew = async (): Promise<AnimeMedia[]> => {
  const res = await fetchWithTimeout(`${NEW_API_URL}/movies`);
  const data = await res.json();
  const movies = data.sections?.[0]?.movies || [];
  return movies.map(mapSectionMovies);
};

export const fetchTvSeriesNew = async (): Promise<AnimeMedia[]> => {
  const res = await fetchWithTimeout(`${NEW_API_URL}/tv-series`);
  const data = await res.json();
  const movies = data.sections?.[0]?.movies || [];
  return movies.map(mapSectionMovies);
};

export const fetchAnimationNew = async (): Promise<AnimeMedia[]> => {
  const res = await fetchWithTimeout(`${NEW_API_URL}/ranking`);
  const data = await res.json();
  const movies = data.sections?.[0]?.movies || [];
  return movies.map(mapSectionMovies);
};
