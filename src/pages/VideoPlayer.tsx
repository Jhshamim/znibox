...
const CORS_PROXY = 'https://frosty-limit-b0fc.jhshamim81.workers.dev/proxy?url=';
...
const VideoPlayer = () => {
  ...
  const searchParamsEpId = searchParams.get('epId');
  const epId = searchParamsEpId || id;
  const episode = parseInt(searchParams.get('ep') || '0', 10);
  ...
  const fetchCacheRef = useRef<any>(null);
  ...
  const fetchStream = useCallback(async () => {
    ...
    if (
      ...
      fetchCacheRef.current.id === id &&
      fetchCacheRef.current.subjectId === subjectId &&
      fetchCacheRef.current.se === se &&
      fetchCacheRef.current.episode === episode
    ) {
      return fetchCacheRef.current.data;
    }
    ...
    try {
      ...
      setStreamData(json);
      const serverList = json.sources.map((src: any) => ({
        type: src.format,
        data_id: src.id,
        server_id: String(src.resolution),
        serverName: src.resolution,
        url: src.url
      }));
      setServers(serverList);
      setHasDub(false); // Managed by language selector outside now
      ...
      fetchCacheRef.current = { id, subjectId, se, episode, data };
      return data;
    } catch (err) {
  ...
  }, [id, subjectId, se, episode]);

  useEffect(() => {
    if (!playerContainerRef.current || !subjectId || !progressLoaded) return;
      ...
      const art = new Artplayer({
        ...
        customType: {
          m3u8: function (video: HTMLVideoElement, url: string) {
                if (Hls.isSupported()) {
                  let proxiedUrl = CORS_PROXY + encodeURIComponent(url);
                  if (result.referer) {
                      try {
                          const originStr = new URL(result.referer).origin;
                          proxiedUrl += '&referer=' + encodeURIComponent(result.referer) + '&origin=' + encodeURIComponent(originStr);
                      } catch (e) {
                    ...
                    xhrSetup: (xhr: XMLHttpRequest, xhrUrl: string) => {
                      ...
                      if (!finalUrl.startsWith(CORS_PROXY)) {
                          finalUrl = CORS_PROXY + encodeURIComponent(xhrUrl);
                          if (result.referer) {
                              try {
                                  const originStr = new URL(result.referer).origin;
                                  finalUrl += '&referer=' + encodeURIComponent(result.referer) + '&origin=' + encodeURIComponent(originStr);
                              } catch (e) {
                                  finalUrl += '&referer=' + encodeURIComponent(result.referer);
                              }
                          }
                      }
                    ...
                    {filteredServers.map((server, index) => (
                      <button
                        ...
                        className={`w-full px-4 py-2.5 text-left text-sm hover:bg-white/10 transition-colors ${
                          currentServer === server.serverName.toLowerCase() || (!filteredServers.some(s => s.serverName.toLowerCase() === currentServer) && index === 0) ? 'text-primary bg-white/5' : ''
                        }`}
                      >
                        {server.serverName}
                      </button>
