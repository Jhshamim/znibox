...
import { useAnimeDetails, useAnimeEpisodes, useAnimeLanguages } from '@/hooks/useAnime';
...
const EpisodeSelector = () => {
  ...
  const { data: languagesData, isLoading: languagesLoading } = useAnimeLanguages(id!);
  const { getAnimeProgress, getEpisodeProgress, formatProgress } = useWatchHistory();
  const [selectedEpisode, setSelectedEpisode] = useState<{season: number, number: number, hasDub: boolean, savedProgress?: WatchHistoryEntry} | null>(null);
    ...
    useEffect(() => {
      const loadProgress = async () => {
        ...
        if (episodesData) {
          ...
          for (const season of episodesData.seasons) {
            for (const ep of season.episodes) {
              const epProgress = await getEpisodeProgress(id, ep.ep);
              if (epProgress) {
                progressMap.set(ep.ep, epProgress);
              }
            }
          }
  ...
  const dubCount = languagesData?.dubs.length || 0;

  const currentSeasonData = useMemo(() => {
    if (!episodesData || !episodesData.seasons) return null;
    return episodesData.seasons[selectedSeason] || episodesData.seasons[0];
  ...
  const handleEpisodeClick = (epData: {season: number, number: number, hasDub: boolean, savedProgress?: WatchHistoryEntry}) => {
    setSelectedEpisode(epData);
    
    if (epData.savedProgress && epData.savedProgress.timestamp > 30) {
      enterFullscreenLandscape();
      // Resume from last known language if possible (we default to the specific subject for now)
      navigate(`/watch/${id}?subjectId=${episodesData.subject_id}&se=${epData.season}&ep=${epData.number}`);
      return;
    }
    
    if (epData.hasDub && languagesData?.dubs) {
      setShowAudioDialog(true);
    } else {
      handlePlay(epData, languagesData?.dubs?.[0] || null);
    }
  };
  ...
  const handlePlay = async (epData: {season: number, number: number}, language: {detailPath: string, subjectId: string} | null) => {
    setShowAudioDialog(false);
    await enterFullscreenLandscape();
    const useSlug = language ? language.detailPath : id;
    const useSubjectId = language ? language.subjectId : episodesData.subject_id;
    navigate(`/watch/${useSlug}?subjectId=${useSubjectId}&se=${epData.season}&ep=${epData.number}`);
  };
  ...
  const handleContinueWatching = async () => {
    ...
    if (episodesData && episodesData.seasons) {
      for (const season of episodesData.seasons) {
        if (season.episodes.some(ep => ep.ep === animeProgress.episode)) {
          targetSe = season.season;
          break;
        }
      }
    }
    
    navigate(`/watch/${id}?subjectId=${episodesData?.subject_id || ''}&se=${targetSe}&ep=${animeProgress.episode}`);
  };
      ...
      <div className="min-h-screen bg-background">
        ...
        <div className="container py-4 sm:py-6 pb-24 sm:pb-32">
          ...
          <div className="flex flex-col gap-4 mb-4 sm:mb-6">
            <div className="flex items-center justify-between">
              ...
              {episodesData.total_seasons > 1 && (
                <div className="flex items-center gap-2 mt-6">
                  <span className="text-xs sm:text-sm font-medium text-muted-foreground">Season:</span>
                  <Select
                  ...
                  >
                    <SelectTrigger className="w-[160px] sm:w-[220px] h-10 sm:h-11 glass-strong border-primary/20">
                      ...
                      <SelectContent 
                        ...
                        position="popper"
                      >
                      {episodesData.seasons.map((season, index) => (
        ...
        <div className="flex flex-wrap items-center gap-4 sm:gap-6 mt-4 sm:mt-6 text-xs sm:text-sm text-muted-foreground">
          ...
          <div className="flex items-center gap-1.5 sm:gap-2">
            ...
            <span>Multiple Audio Available</span>
          </div>
        </div>
      </div>
          ...
          >
            ...
            >
              <div className="p-4 sm:p-6">
                <h3 className="text-base sm:text-lg font-bold mb-1.5 sm:mb-2">Episode {selectedEpisode.number}</h3>
                <p className="text-xs sm:text-sm text-muted-foreground mb-4 sm:mb-6">
                  Choose your preferred audio/sub
                </p>

                <div className="space-y-2 sm:space-y-3 max-h-[50vh] overflow-y-auto">
                  {languagesData?.dubs.map((dub, index: number) => (
                    <motion.button
                      key={dub.subjectId + index}
                    ...
                    >
                      <div className="flex items-center gap-2.5 sm:gap-3">
                        <div className={`w-9 h-9 sm:w-10 sm:h-10 rounded-full ${dub.original ? 'bg-primary/20' : 'bg-accent/20'} flex items-center justify-center`}>
                          {dub.original ? <Volume2 className={`w-4 h-4 sm:w-5 sm:h-5 text-primary`} /> : <VolumeX className={`w-4 h-4 sm:w-5 sm:h-5 text-accent`} />}
                        </div>
                        <div className="text-left">
                          <p className="font-semibold text-sm sm:text-base">{dub.lanName}</p>
                          <p className="text-[10px] sm:text-xs text-muted-foreground">{dub.original ? 'Original Audio' : (dub.type === 1 ? 'Subtitle' : 'Dubbed')}</p>
                        </div>
                      </div>
